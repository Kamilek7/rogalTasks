...
function App() {
  ...
  const pobierzZadania = async (date) =>   {
    const response = await fetch("https://tasks-backend.rogal-rogal.duckdns.org/zadania/"+date);
    const data = await response.json();
    pobierz(data.zadania);
  }

    const pobierzHarmonogram = async () =>   {
        const response = await fetch("https://tasks-backend.rogal-rogal.duckdns.org/harmonogram/1");
        const data = await response.json();
  ...
  return <>
    ...
    <div id='buttonsLayout'>
      ...
      <button onClick={ () => {console.log("wyloguj")}}><i class="icon-logout"></i></button>
    </div>
